...
class GreetingState(object):
    ...
    def _get_linked_message(self, name: str) -> models.LinkedMessages:
        return models.LinkedMessages.objects.get(name=name)

    def _prepare_context(self):
        linked_message: models.LinkedMessages = self._get_linked_message(
            self.linked_message_name)
        self.context = message.MessageContext(self.bot, linked_message)

    def _next_state_handler(self, user: models.User):
...
class DashboardState(object):
    ...
    def _next_state_handler(self, user: models.User, task: str):
        user.state = self.next_state
        user.substate = None
    ...
    def _finish_state_handler(self, user: models.User):
    ...
    async def incoming_handler(self, user: models.User, text: str, message_id: int):
        ...
        if result is not None:
            self._next_state_handler(user, result)
    ...
    async def outcoming_handler(self, user: models.User, not_done_tasks: Optional[List[str]] = None):
        ...
        if not_done_tasks is None:
            not_done_tasks: List[str] = self._get_not_done_tasks(user)
            
        if len(not_done_tasks) == 0:
            self._finish_state_handler(user)
    ...
    def _get_not_done_tasks(self, user: models.User) -> List[str]:
        all_tasks: List[models.FreeAnswerQuiz] = models.FreeAnswerQuiz.objects.all()
...
class QuizState(object):
    ...
    def _get_quiz(self, name: str) -> models.FreeAnswerQuiz:
        return models.FreeAnswerQuiz.objects.get(name=name)

    def _prepare_context(self):
        quiz: models.FreeAnswerQuiz = self._get_quiz(self.task_name)
        self.context = message.QuizContext(self.bot, quiz)

    def _back_to_menu(self, user: models.User) -> None:
    ...
    async def run_incoming(self, user: models.User, test: str):
        ...
        if text_type == 'back':
            ...
            self.right_answer_handler(user)
            return True
        return None
...
class Machine(object):
    ...
    def _create_new_user(self, tg_message: types.Message) -> None:
        user: models.User = models.User.create(tg_message)
        user.save()
        return user

    def _get_user_by_message(self, tg_message: types.Message) -> Optional[models.User]:
        chat_id: int = tg_message.chat.id
        user_object: models.User = None
        try:
            user_object = models.User.objects.get(chat_id=chat_id)
        except models.User.DoesNotExist:
            user_object = self._create_new_user(tg_message)
        return user_object
    ...
    async def message_handler(self, tg_message: types.Message) -> None:
        ...
        user: models.User = self._get_user_by_message(tg_message)
    ...
    async def callback_handler(self, callback_query: types.CallbackQuery):
        user: models.User = self._get_user_by_message(callback_query.message)
